...
rewardsApi.prototype.getRewards = async function (user, pass, isMobile = false) {
    ...
    browser.then(async browser => {
        ...
        if(level === 2) {
            console.log("Level 2");          
            await getSearchLinks(page, 40, false);
            await getSearchLinks(page, 30, true);
        }
        else if(level === 1) {
            console.log("Level 1");                        
            await getSearchLinks(page, 1, false);
        }
        await browser.close();
    });
};
...
async function getSearchLinks(page, nbSearch = 10, isMobile = false) {
    ...
    await asyncForEach(searchTerms, async(term) => {
        ...
        //await page.click('ol[id=b_results] a:first-child');
        /*await page.waitForNavigation({
            waitUntil: 'load'
        });*/
    });
